...
DISK_USAGE=$(df -h /host_root | awk 'NR==2 {print $5}')
...
# /host_proc/net/dev 的第2列是接收字节数(rx_bytes), 第10列是发送字节数(tx_bytes)
CURRENT_STATS=$(grep "${INTERFACE}:" /host_proc/net/dev | awk '{print $2, $10}')
if [ -n "$CURRENT_STATS" ]; then
    ...
    if [ -f "$STATS_FILE" ]; then
        ...
        if [ "$TIME_DIFF" -gt 0 ]; then
            ...
            NET_RX_RATE=$(echo "scale=2; $RX_BYTES_DIFF / $TIME_DIFF / 1024" | bc)
            NET_TX_RATE=$(echo "scale=2; $TX_BYTES_DIFF / $TIME_DIFF / 1024" | bc)
        fi
    fi

    # 将当前数据写入文件，供下一次使用
    echo "$CURRENT_TIME $CURRENT_RX_BYTES $CURRENT_TX_BYTES" > "$STATS_FILE"
else
...
"$CPU_USAGE" "$MEM_USAGE" "$DISK_USAGE" "$NET_RX_RATE" "$NET_TX_RATE")
...
# 执行推送
curl -s --fail "$FINAL_URL" > /dev/null
